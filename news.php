...
<head>
	...
	<?php
		...
		$result = mysql_query("SELECT DATE_FORMAT(news_date, '%M %e, %Y - %h:%i %p') AS display_date, news_title, news_content, news_image, news_file, news_file_link_text FROM news WHERE news_id>0 ORDER BY news_date DESC", $link);
		if (!$result) {
		...
		while ($row = mysql_fetch_array($result, MYSQL_BOTH)) {
			...
			$news .= "<hr width=\"50%\">\n";
			$news .= "<h3 align=\"center\">".$title."</h3>\n";
			if (!empty($image))
				{
				$news .= "<p align=\"center\"><a class=\"zoom\" href=\"news/".$image."\"><img src=\"news/thumbs/".$image."\" /></a></p>\n";
				}
			$news .= "<p align=\"center\">".$date."</p>\n";
			$news .= "<p align=\"center\">".$content."</p>\n";
			if (!empty($newsFile))
				{
